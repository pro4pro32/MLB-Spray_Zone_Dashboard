...
# ════════════════════════════════════════════════════════════
DATA_DIR    = Path(".")
AVAIL_YEARS = [y for y in range(2021, 2026)
...
"English": dict(
    ...
    more_filters="You can find more filters on the sidebar.",
    tab_main="Main",
    metric_whiff="Whiff%",
    metric_swing="Swing%",
    metric_contact="Contact%",
    metric_avg_la="Average Launch Angle",
    metric_avg_ev="Average Exit Velocity",
...
# ── Apply pitch-group + pitch + count filters ─────────────────
df_work_pre = df_base.copy()

# Selecting a group immediately filters the whole dashboard to that group.
# Individual pitch selections then narrow it further.
if HAS_PITCH:
    if group_sel == T["group_fb"]:
        df_work_pre = df_work_pre[df_work_pre["pitch_type"].isin(PITCH_GROUPS["Fastball"])]
    elif group_sel == T["group_os"]:
        df_work_pre = df_work_pre[df_work_pre["pitch_type"].isin(PITCH_GROUPS["Offspeed"])]
    elif group_sel == T["group_br"]:
        df_work_pre = df_work_pre[df_work_pre["pitch_type"].isin(PITCH_GROUPS["Breaking"])]
...
def main_metrics(df, zone):
    """Return pitch-level swing/whiff/contact rates and batted-ball averages."""
    sub = _sub(df, zone)
    result = {
        "whiff_pct": np.nan,
        "swing_pct": np.nan,
    ...
    desc = (sub["description"].astype(str).str.lower()
            if "description" in sub.columns
            else pd.Series("", index=sub.index))

    # Statcast swing events: all contact events plus swing-and-miss / missed-bunt attempts.
    swing_events = {
        "foul", "foul_tip", "foul_bunt", "bunt_foul",
        "hit_into_play", "hit_into_play_no_out", "hit_into_play_score",
        "swinging_strike", "swinging_strike_blocked",
        "missed_bunt_attempt",
    }
    whiff_events = {"swinging_strike", "swinging_strike_blocked", "missed_bunt_attempt"}
    contact_events = swing_events - whiff_events

    swing_mask = desc.isin(swing_events)
    whiff_mask = desc.isin(whiff_events)
    contact_mask = desc.isin(contact_events)

    result["swings"] = int(swing_mask.sum())
    result["whiffs"] = int(whiff_mask.sum())
    result["contacts"] = int(contact_mask.sum())
    result["bip"] = int(sub["launch_angle"].notna().sum()) if "launch_angle" in sub.columns else 0

    pitches = result["pitches"]
    swings = result["swings"]
    if pitches:
        result["swing_pct"] = result["swings"] / pitches * 100.0
    if swings:
        result["whiff_pct"] = result["whiffs"] / swings * 100.0
        result["contact_pct"] = result["contacts"] / swings * 100.0

    if "launch_angle" in sub.columns:
        la = to_f(sub["launch_angle"]).dropna()
        if not la.empty:
            result["avg_la"] = float(la.mean())
    if "launch_speed" in sub.columns:
        ev = to_f(sub["launch_speed"]).dropna()
        if not ev.empty:
            result["avg_ev"] = float(ev.mean())
    return result
...
with col_right:
    ...
    tab_main, tab_st, tab_sp, tab_la, tab_jt, tab_mv = st.tabs([
        T["tab_main"], T["tab_stats"], T["tab_spray"], T["tab_la"],
        T["tab_joint"], T["tab_move"],
    ])

    with tab_main:
        try:
            m = main_metrics(df_work, sel_zone)
            k1, k2, k3, k4, k5 = st.columns(5)
            with k1:
                st.metric(T["metric_whiff"],
                          f"{m['whiff_pct']:.1f}%" if not np.isnan(m["whiff_pct"]) else "—")
            with k2:
                st.metric(T["metric_swing"],
                          f"{m['swing_pct']:.1f}%" if not np.isnan(m["swing_pct"]) else "—")
            with k3:
                st.metric(T["metric_contact"],
                          f"{m['contact_pct']:.1f}%" if not np.isnan(m["contact_pct"]) else "—")
            with k4:
                st.metric(T["metric_avg_la"],
                          f"{m['avg_la']:.1f}°" if not np.isnan(m["avg_la"]) else "—")
            with k5:
                st.metric(T["metric_avg_ev"],
                          f"{m['avg_ev']:.1f} mph" if not np.isnan(m["avg_ev"]) else "—")

            st.caption(
                f"Pitches: {m['pitches']:,} · Swings: {m['swings']:,} · "
                f"Whiffs: {m['whiffs']:,} · Contact: {m['contacts']:,} · BIP: {m['bip']:,}"
            )
        except Exception as e:
            st.error(str(e))
